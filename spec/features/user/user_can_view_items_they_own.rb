...
feature 'user can view items they own' do
  it 'user can view a file in their root director' do
    user = create(:user)
    ...
    expect(page).to have_contents(upload.name)
    expect(page).to have_contents(upload.size)
    expect(page).to have_contents(upload.file_type)
  end

  it 'user can view the contents of a root directory' do
    user   = create(:user)
  ...
  it 'user can view a sub-file' do
    user = create(:user)
  ...
  it 'user can view the contents of a sub-folder' do
    user    = create(:user)
    root    = create(:folder, user: user)
